...
local function retrieveTimeFromApi(callback)
    Citizen.CreateThread(function()
        PerformHttpRequest("https://api.ipify.org?format=json", function(statusCode, response)
    print("[TestAPI] Status:", statusCode)
    print("[TestAPI] Response:", response)
end)

            print("[TimeAPI] Status Code:", statusCode)
            if statusCode == 200 then
                local data = json.decode(response)
                if data == nil or data.unixtime == nil then
                    print("[TimeAPI] Failed to parse response or missing unixtime")
                    callback(nil)
                else
                    print("[TimeAPI] Time retrieved:", data.unixtime)
                    callback(data.unixtime)
                end
            else
                print("[TimeAPI] HTTP request failed")
                callback(nil)
            end
        end, "GET", nil, nil)
    end)
