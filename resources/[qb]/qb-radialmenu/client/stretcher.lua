...
local function setClosestStretcher()
    ...
    local object = GetClosestObjectOfType(coords.x, coords.y, coords.z, 10.0, `prop_med_stretcher`, false, false, false)
    if object ~= 0 then
...
RegisterNetEvent('qb-radialmenu:client:TakeStretcher', function()
    ...
    if vehicle ~= 0 then
        RequestModel("prop_med_stretcher")
        while not HasModelLoaded("prop_med_stretcher") do
            Wait(0)
        end
        local obj = CreateObject(`prop_med_stretcher`, GetEntityCoords(PlayerPedId()), true)
        if obj ~= 0 then
