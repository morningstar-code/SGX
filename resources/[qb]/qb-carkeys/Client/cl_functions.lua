...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    print("Updating OutsideVehicles")
    QBCore.Functions.TriggerCallback("qb-carkeys:server:GetOutsideVehicles", function(result)
        if result then OutsideVehicles = result end
    end)
end)

RegisterCommand("orihashamen", function()
    for k,v in ipairs(OutsideVehicles) do
        print(k,v)
    end
        ...
        if v.showBlip then
            if job == v.job then
                if v.isHelipad then
                    ...
                    SetBlipSprite (Helipad, 290)
                    SetBlipDisplay(Helipad, 4)
                    SetBlipScale  (Helipad, 0.5)
                    SetBlipAsShortRange(Helipad, true)
                    SetBlipColour(Helipad, 18)
                    BeginTextCommandSetBlipName("STRING")
        ...
        if v.showBlip ~= nil then
            if v.showBlip then
                if state then
                    ...
                    SetBlipSprite (v.blip, 290)
                    SetBlipDisplay(v.blip, 4)
                    SetBlipScale  (v.blip, 0.5)
                    SetBlipAsShortRange(v.blip, true)
                    SetBlipColour(v.blip, 18)
                    BeginTextCommandSetBlipName("STRING")
...
function SpawnVehicle(vehicle, pGarage, Fuel, body, engine, plate, gType, IsViewing, IsShared)
    ...
    if ParkingSpot ~= nil then
        if not IsViewing then
            ...
            QBCore.Functions.TriggerCallback('qb-carkeys:server:GetVehicleProperties', function(properties)
                ...
                local curfuelgarage = GetVehicleFuelLevel(veh)
                exports['sgx-fuel']:SetFuel(veh, curfuelgarage)
            ...
            if IsShared then
                TriggerServerEvent('qb-carkeys:server:updateSharedVehState', 'Out', plate, pGarage)
            else
                TriggerServerEvent('qb-carkeys:server:updateVehicleState', 0, plate, pGarage)
                if plate ~= nil then
                    OutsideVehicles[plate] = CurrentDisplayVehicle
                    TriggerServerEvent('qb-carkeys:server:UpdateOutsideVehicles', OutsideVehicles)
                end
            end
...
function SpawnDepotVehicle(Data)
    ...
    if OutsideVehicles ~= nil and next(OutsideVehicles) ~= nil then
        if OutsideVehicles[Data.plate] ~= nil then
            print(OutsideVehicles[Data.plate])
            -- local VehExists = DoesEntityExist(OutsideVehicles[Data.plate])
            local VehExists = GetVehicleByPlate(Data.plate)
            print(VehExists)
            if VehExists ~= nil then
                if Var.DepotGetDestroyedVeh then
                    local Engine = GetVehicleEngineHealth(OutsideVehicles[Data.plate])
                    if Engine < 40 then
                        if spawnpoint ~= nil then
                            QBCore.Functions.TriggerCallback('qb-carkeys:server:PayDepotPrice', function(paid)
                                if paid then
                                    if OutsideVehicles[Data.plate] then
                                    ...
                                    QBCore.Functions.TriggerCallback('qb-carkeys:server:GetVehicleProperties', function(properties)
                                        ...
                                        SetEntityAsMissionEntity(veh, true, true)
                                        TriggerServerEvent('qb-carkeys:server:updateVehicleState', 0, Data.plate, Data.garage)
