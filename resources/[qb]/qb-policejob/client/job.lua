...
local function TakeOutVehicle(vehicleInfo)
    ...
    if coords then
        ...
        QBCore.Functions.TriggerCallback('police:server:PayForVehicle', function(result, platenr)
            if not result then return QBCore.Functions.Notify(Lang:t('error.not_enough_money'), 'error', 4500) end
            local plate = ''
            QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
                local veh = NetToVeh(netId)
                SetCarItemsInfo()
                if Config.OwnedPoliceCars then plate = platenr else plate = Lang:t('info.police_plate')..tostring(math.random(1000, 9999)) end
                ...
                vehicleMenu[#vehicleMenu+1] = {
                    ...
                    optparams = {
                        ...
                        args = {
                            ...
                            livery = data.livery,
                            out = false, -- Ensure this is set to false
                            price = vehprice
                        }
                    }
                }
...
RegisterNetEvent('police:client:VehicleSubMenu', function(data)
    local vehLabel = data.vehlabel
    local SubMenu = {}
    -- Only add the takeout menu option
    SubMenu[#SubMenu+1] = {
        opthead = Lang:t('menu.takeout', {vehicle = vehLabel}),
        optdesc = Lang:t('menu.takeoutfor', {price = data.price}),
        opticon = 'car',
        optparams = {
...
RegisterNetEvent('police:client:TakeOutVehicle', function(args)
    -- Your logic for taking out the vehicle goes here

    -- After taking out the vehicle, call MenuGarage again with the current selection
    MenuGarage(args.currentSelection)
end)
    ...
    impoundCombo:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            ...
            if PlayerJob.type == "leo" and PlayerJob.onduty and IsPedInAnyVehicle(PlayerPedId(), false) then
                ...
                PolDrawText.Open(Lang:t('info.pol_impound'), Config.DrawText.position)
