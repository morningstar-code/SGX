...
Requested = {
    ...
    [3] = {name = QBCore.Shared.Items["plasmacutter"]["name"], image = QBCore.Shared.Items["plasmacutter"]["image"]},
    [4] = {name = QBCore.Shared.Items["weapon_stickybomb"]["name"], image = QBCore.Shared.Items["weapon_stickybomb"]["image"]},
...
function AlertPolice()
    ...
    TriggerServerEvent("vir_dispatch:SendAlert", {
        jobs = {"police"}, -- Puede ser police, ambulance o ambos separados por coma EJ> {"police", "ambulance"}
        title = "Aviso de central", -- Titulo EJ> Venta de Drogas
        message = "MENSAJE", -- EJ> Se veria una venta de drogas
        coords = GetEntityCoords(PlayerPedId()) -- Coordenadas del jugador
    })	
end
...
function Hacking(index)
    QBCore.Functions.TriggerCallback('pacificheist:server:hasItem', function(hasItem, itemLabel)
        if hasItem then
            ...
            TriggerEvent("sgx_fingerprint:Start", 3, 6, 2, function(outcome, reason)
