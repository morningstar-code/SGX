...
QBCore.Functions.CreateCallback('fleecaheist:server:hasItem', function(source, cb, item)
    ...
    if player and playerItem ~= nil then
       ...
       -- print('[sgx-fleecaheist] you need add required items to server database')
...
AddEventHandler('fleecaheist:server:policeAlert', function(coords)
    local alert = {
        jobs = {}, -- Puede ser police, ambulance o ambos separados por coma EJ> {"police", "ambulance"}
        title = "Aviso de central", -- Titulo EJ> Venta de Drogas
        message = "MENSAJE", -- EJ> Se veria una venta de drogas
        coords = GetEntityCoords(GetPlayerPed(source)) --Coordenadas del jugador
      }
exports.vir_dispatch:addDispatchAlert(alert)
end)
