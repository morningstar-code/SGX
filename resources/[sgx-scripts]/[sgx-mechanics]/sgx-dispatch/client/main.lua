...
local function openMenu()
    ...
    local calls = lib.callback.await('sgx-dispatch:callback:getCalls', false)
    if #calls == 0 then
...
local function setWaypoint()
    ...
    local data = lib.callback.await('sgx-dispatch:callback:getLatestDispatch', false)
    ...
    if not waypointCooldown and lib.table.contains(data.jobs, PlayerData.job.type) then
        ...
        TriggerServerEvent('sgx-dispatch:server:attach', data.id, PlayerData)
...
-- Events
RegisterNetEvent('sgx-dispatch:client:notify', function(data, source)
    if data.alertTime == nil then data.alertTime = Config.AlertTime end
    local timer = data.alertTime * 1000
...
RegisterNetEvent('sgx-dispatch:client:openMenu', function(data)
    if not isJobValid(PlayerData.job.type) then return end
...
RegisterNUICallback("detachUnit", function(data, cb)
    TriggerServerEvent('sgx-dispatch:server:detach', data.id, PlayerData)
    DeleteWaypoint()
    cb("ok")
end)
...
RegisterNUICallback("refreshAlerts", function(data, cb)
    ...
    local data = lib.callback.await('sgx-dispatch:callback:getCalls', false)
    SendNUIMessage({ action = 'setDispatchs', data = data, })
    cb("ok")
end)
