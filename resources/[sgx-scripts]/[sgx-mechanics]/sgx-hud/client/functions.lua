...
function Koci.Client:TriggerServerCallback(key, payload, func)
    ...
    TriggerServerEvent("sgx-hud:Server:HandleCallback", key, payload)
end
...
function Koci.Client.HUD:GetFuelExport()
    ...
    elseif GetResourceState("sgx-fuel") == "started" then
        if Config.FrameWork == "esx" then
            return Koci.Framework.Math.Round(exports["sgx-fuel"]:GetFuel(self.data.vehicle.entity) or 0, 2)
        elseif Config.FrameWork == "qb" then
            return Koci.Framework.Shared.Round(exports["sgx-fuel"]:GetFuel(self.data.vehicle.entity) or 0, 2)
        end
