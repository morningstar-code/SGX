...
RegisterNetEvent('sgx-rental:sv:rentVehicle', function(data)
    local src = source
    ...
    if data.payType == "cash" then
        if Player.Functions.GetMoney(data.payType) >= data.carPrice then
            ...
            TriggerClientEvent('sgx-rental:cl:spawnVehicle', src, data.carName, data.carDay)
...
RegisterNetEvent('sgx-rental:sv:updatesql', function(plate,vehicle,time)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local cid = Player.PlayerData.citizenid
...
RegisterNetEvent('sgx-rental:sv:checktime', function()
