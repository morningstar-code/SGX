...
local MugShotWebhook = 'https://discord.com/api/webhooks/1236040472267329648/HoCDOTldi2wun--NV-KgeWcIAK610zzy_cSmjiF53XAKZqoKOejpVQwoxjC9N_eMwaf4'

-- Clock-in notifications for duty. Add a Discord webhook.
-- Command /mdtleaderboard, will display top players per clock-in hours.
local ClockinWebhook = 'https://discord.com/api/webhooks/1236040472267329648/HoCDOTldi2wun--NV-KgeWcIAK610zzy_cSmjiF53XAKZqoKOejpVQwoxjC9N_eMwaf4'

-- Incident and Incident editting. Add a Discord webhook.
-- Incident Author, Title, and Report will display in webhook post.
local IncidentWebhook = 'https://discord.com/api/webhooks/1236040472267329648/HoCDOTldi2wun--NV-KgeWcIAK610zzy_cSmjiF53XAKZqoKOejpVQwoxjC9N_eMwaf4'
--------------------------------

QBCore.Functions.CreateCallback('sgx-mdt:server:MugShotWebhook', function(source, cb)
...
RegisterServerEvent("sgx-mdt:dispatchStatus", function(bool)
...
AddEventHandler('onResourceStart', function(resourceName)
	...
	-- if GetResourceState('sgx-dispatch') == 'started' then
	-- 	local calls = exports['sgx-dispatch']:GetDispatchCalls()
	-- 	return calls
	-- end
end)

RegisterNetEvent("sgx-mdt:server:OnPlayerUnload", function()
...
RegisterNetEvent("sgx-mdt:server:ToggleDuty", function()
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
...
QBCore.Commands.Add("mdtleaderboard", "Show MDT leaderboard", {}, false, function(source, args)
    ...
    sendToDiscord(16753920, "MDT Leaderboard", leaderboard_message, "sgx-mdt | Made by Project Sloth")
    TriggerClientEvent('QBCore:Notify', source, "MDT leaderboard sent to Discord!", 'success')
end)

RegisterNetEvent("sgx-mdt:server:ClockSystem", function()
    local src = source
    local PlayerData = GetPlayerData(src)
    local time = os.date("%Y-%m-%d %H:%M:%S")
    local firstName = PlayerData.charinfo.firstname:sub(1,1):upper()..PlayerData.charinfo.firstname:sub(2)
...
RegisterNetEvent('mdt:server:openMDT', function()
	...
	-- if GetResourceState('sgx-dispatch') == 'started' then
	-- 	calls = exports['sgx-dispatch']:GetDispatchCalls()
	-- end
...
QBCore.Functions.CreateCallback('sgx-mdt:getDispatchCalls', function(source, cb)
    local calls = exports['sgx-dispatch']:GetDispatchCalls()
    cb(calls)
end)
					...
					if impoundInfo.impoundActive then
						...
						if vehicle.impoundid ~= nil then
							...
							if result then
								...
								result.plate = plate
								TriggerClientEvent('sgx-mdt:client:TakeOutImpound', src, result)
							end
                        ...
                        MySQL.Async.fetchAll('SELECT `author`, `title`, `details` FROM `mdt_incidents` WHERE `id` = @id', { ['@id'] = infoResult }, function(result)
                            if result and #result > 0 then
                                ...
                                for i=1, #associated do
                                    ...
                                    sendIncidentToDiscord(3989503, "MDT Incident Report", message, "sgx-mdt | Made by Project Sloth", associatedData)                                
                                end
                            else
                ...
                MySQL.Async.fetchAll('SELECT `author`, `title`, `details` FROM `mdt_incidents` WHERE `id` = @id', { ['@id'] = id }, function(result)
                    if result and #result > 0 then
                        ...
                        for i=1, #associated do
                            ...
                            sendIncidentToDiscord(16711680, "MDT Incident Report has been Updated", message, "sgx-mdt | Made by Project Sloth", associatedData)
                        end
                    else
...
RegisterNetEvent('mdt:server:registerweapon', function(serial, imageurl, notes, owner, weapClass, weapModel) 
    exports['sgx-mdt']:CreateWeaponInfo(serial, imageurl, notes, owner, weapClass, weapModel)
end)
