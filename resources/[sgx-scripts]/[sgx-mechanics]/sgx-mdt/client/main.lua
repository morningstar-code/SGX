...
CreateThread(function()
    if GetResourceState('sgx-dispatch') == 'started' then
        TriggerServerEvent("sgx-mdt:dispatchStatus", true)
    end
end)
...
RegisterNetEvent('QBCore:Client:OnPlayerUnload', function()
    TriggerServerEvent("sgx-mdt:server:OnPlayerUnload")
    PlayerData = {}
end)
...
RegisterNetEvent("QBCore:Client:SetDuty", function(job, state)
    if AllowedJob(job) then
        TriggerServerEvent("sgx-mdt:server:ToggleDuty")
	TriggerServerEvent("sgx-mdt:server:ClockSystem")
...
RegisterNUICallback("removeCallBlip", function(data, cb)
    TriggerEvent('sgx-dispatch:client:removeCallBlip', data.callid or data.id)
    cb(true)
end)
...
RegisterNetEvent('sgx-mdt:client:selfregister')
AddEventHandler('sgx-mdt:client:selfregister', function()
...
if Config.UseWolfknightRadar == true then
    RegisterNetEvent("sgx-mdt:client:trafficStop")
    AddEventHandler("sgx-mdt:client:trafficStop", function()
        ...
        if not coolDown then
            if data.veh ~= nil and data.veh ~= 0 then
                ...
                exports["sgx-dispatch"]:CustomAlert({
                    coords = {
                        x = currentPos.x,
                        y = currentPos.y,
                        z = currentPos.z
