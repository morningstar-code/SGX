...
QBCore.Functions.CreateUseableItem(Config.LaptopDevice, function(source, item)
    ...
    if Player.Functions.GetItemByName(Config.LaptopDevice) then
        TriggerClientEvent('sgx-laptop:client:openlaptop', source)
    end
end)
...
RegisterNetEvent('sgx-laptop:server:LostAccess', function(app)
    local src = source
    if app == "boosting" then
        TriggerEvent("sgx-laptop:server:QuitQueue", src)
    end
end)


RegisterNetEvent('sgx-laptop:server:settings:set', function(setting)
    local src = source
...
RegisterNetEvent('sgx-laptop:server:RemoveItem', function(item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
AddEventHandler('onResourceStart', function(resource)
    ...
    lib.versionCheck('Ejiy/sgx-laptop')
end)
