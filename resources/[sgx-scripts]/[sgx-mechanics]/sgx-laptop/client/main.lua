                        ...
                        for k = 1, searches do
                            if app.bannedJobs[k] == playerJob then
                                if hadApp(app.app) then
                                    TriggerServerEvent('sgx-laptop:server:LostAccess', app.app)
                                end
                                goto skip
...
RegisterNetEvent('sgx-laptop:client:openlaptop', function()
        SetDisplay(true)
end)

RegisterNetEvent('sgx-laptop:client:CustomNotification', function(text, icon, color, iconbg, length)
...
RegisterNUICallback('laptop/checkout', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('sgx-laptop:server:checkout', function(result)
        if result == "bank" then
            cb({
                status = 'error',
...
RegisterNUICallback('setting/save', function(data, cb)
    ...
    cb("ok")
    TriggerServerEvent("sgx-laptop:server:settings:set", data["setting"])
end)
