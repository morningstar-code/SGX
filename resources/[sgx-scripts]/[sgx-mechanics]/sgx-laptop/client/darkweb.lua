...
local function openCrate(crate)
    ...
    QBCore.Functions.TriggerCallback('sgx-laptop:server:getCrateStatus', function(data)
...
local function breakCrate(entity)
    if haveItem('drill') then
        exports['sgx-skillchecks']:untangle(function(success)
            if success then
                TriggerServerEvent('sgx-laptop:server:crateOpened', NetworkGetNetworkIdFromEntity(entity))
                if crateBlip then
                    RemoveBlip(crateBlip)
                end
            end
        end, "Black Market", "AirDrop!", 20000, 8) -- Time, Grid size (5, 6, 7, 8, 9, 10), Incorrect blocks
    else
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    QBCore.Functions.TriggerCallback('sgx-laptop:server:getAllCrates', function(crateInfo)
        crates = crateInfo
...
RegisterNetEvent('sgx-laptop:client:updateCrates', function(crateInfo)
    crates = crateInfo
end)
