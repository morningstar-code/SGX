...
RegisterServerEvent('sgx-payments:server:ATM:use', function(amount, billtype, baccount, account, society, gsociety)
	local src = source
	...
	elseif account == "society" then
		if billtype == "withdraw" then
			...
			elseif tonumber(society) >= amount then
				...
				elseif Config.Banking == "sgx-banking" then exports["sgx-banking"]:RemoveMoney(tostring(Player.PlayerData.job.name), amount)
		...
		elseif billtype == "deposit" then
			...
			elseif bankB >= amount then
				...
				elseif Config.Banking == "sgx-banking" then exports["sgx-banking"]:AddMoney(tostring(Player.PlayerData.job.name), amount)
	...
	elseif account == "gang" then
		if billtype == "withdraw" then
			...
			elseif tonumber(gsociety) >= amount then
				...
				elseif Config.Banking == "sgx-banking" then exports["sgx-banking"]:RemoveGangMoney(tostring(Player.PlayerData.gang.name), amount)
		...
		elseif billtype == "deposit" then
			...
			elseif bankB >= amount then
				...
				elseif Config.Banking == "sgx-banking" then exports["sgx-banking"]:AddGangMoney(tostring(Player.PlayerData.gang.name), amount)
...
QBCore.Functions.CreateCallback('sgx-payments:ATM:Find', function(source, cb)
	...
	if Config.Banking == "sgx-banking" then
...
QBCore.Commands.Add("cashgive", Loc[Config.Lan].command["pay_user"], {}, false, function(source) TriggerClientEvent("sgx-payments:client:ATM:give", source) end)

RegisterServerEvent("sgx-payments:server:ATM:give", function(citizen, price)
    local Player = QBCore.Functions.GetPlayer(source)
