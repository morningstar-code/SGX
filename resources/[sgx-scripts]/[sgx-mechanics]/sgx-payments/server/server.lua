...
AddEventHandler('onResourceStart', function(resource) if GetCurrentResourceName() ~= resource then return end
	for k in pairs(Config.Jobs) do
		if not QBCore.Shared.Jobs[k] and not QBCore.Shared.Gangs[k] then
			print("sgx-payments: Config.Jobs searched for job/gang '"..k.."' and couldn't find it in the Shared")
		end
	end
end)
...
CreateThread(function()
    if Config.Enablecommand then
        QBCore.Commands.Add("cashregister", Loc[Config.Lan].command["cash_reg"], {}, false, function(source) TriggerClientEvent("sgx-payments:client:Charge", source, {}, true) end)
    end
end)

QBCore.Commands.Add("polcharge", Loc[Config.Lan].command["charge"], {}, false, function(source) TriggerClientEvent("sgx-payments:client:PolCharge", source) end)

RegisterServerEvent('sgx-payments:Tickets:Give', function(data, biller, gang)
	...
	if biller then -- If this is found, it ISN'T a phone payment, so add money to society here
		if gang then
			...
			elseif Config.Banking == "sgx-banking" then
				exports["sgx-banking"]:AddGangMoney(tostring(biller.PlayerData.gang.name), data.amount - takecomm)
				if Config.Debug then print("^5Debug^7: ^3sgx-banking^7(^3Gang^7): ^2Adding ^7$^6"..data.amount - takecomm.." ^2to account ^7'^6"..tostring(biller.PlayerData.gang.name).."^7' ($^6"..exports["sgx-banking"]:GetGangAccount(biller.PlayerData.gang.name).."^7)") end
		...
		elseif not gang then
			...
			elseif Config.Banking == "sgx-banking" then
				exports["sgx-banking"]:AddMoney(tostring(biller.PlayerData.job.name), data.amount - takecomm)
				if Config.Debug then print("^5Debug^7: ^3sgx-banking^7(^3Job^7): ^2Adding ^7$^6"..data.amount - takecomm.." ^2to account ^7'^6"..tostring(biller.PlayerData.job.name).."^7' ($^6^7)") end
			elseif Config.Banking == "fd" then
...
RegisterServerEvent('sgx-payments:Tickets:Sell', function()
...
QBCore.Functions.CreateCallback('sgx-payments:Ticket:Count', function(source, cb)
	cb(QBCore.Functions.GetPlayer(source).Functions.GetItemByName('payticket'))
end)

RegisterServerEvent("sgx-payments:server:Charge", function(citizen, price, billtype, img, outside, gang)
	local src = source
	...
	if amount and amount > 0 then
		...
		if Config.PhoneBank == false or gang == true or billtype == "cash" then
			TriggerClientEvent("sgx-payments:client:PayPopup", billed.PlayerData.source, amount, src, billtype, img, label, gang, outside)
		else
...
RegisterServerEvent("sgx-payments:server:PayPopup", function(data)
	local src = source
	...
	if data.accept == true then
		...
		TriggerEvent('sgx-payments:Tickets:Give', newdata, biller, data.gang)
...
RegisterServerEvent("sgx-payments:server:PolCharge", function(citizen, price)
	local src = source
    local biller = QBCore.Functions.GetPlayer(src)
    local billed = QBCore.Functions.GetPlayer(tonumber(citizen))
	...
	if price > 0 then
		if not Config.FineJobConfirmation then
			...
			elseif Config.Banking == "sgx-banking" then
				exports["sgx-banking"]:AddMoney(tostring(biller.PlayerData.job.name), (price - commission))
				if Config.Debug then print("^5Debug^7: ^3sgx-banking^7(^3Job^7): ^2Adding ^7$^6"..(price - takecomm).." ^2to account ^7'^6"..tostring(biller.PlayerData.job.name).."^7' ($^6"..exports["sgx-banking"]:GetAccount(biller.PlayerData.job.name).."^7)") end
			elseif Config.Banking == "fd" then
			...
			TriggerClientEvent("sgx-payments:client:PolPopup", billed.PlayerData.source, price, src, biller.PlayerData.job.label)
		end
	else triggerNotify(nil, Loc[Config.Lan].error["charge_zero"], 'error', source) return end
end)

RegisterServerEvent("sgx-payments:server:PolPopup", function(data)
	local src = source
    local billed = QBCore.Functions.GetPlayer(src)
    local biller = QBCore.Functions.GetPlayer(tonumber(data.biller))
	...
	if data.accept == true then
		...
		elseif Config.Banking == "qb-management" then
			...
			if Config.Debug then print("^5Debug^7: ^3QB-Management^7(^3Job^7): ^2Adding ^7$^6"..data.amount - takecomm.." ^2to account ^7'^6"..tostring(biller.PlayerData.job.name).."^7' ($^6"..exports["qb-management"]:GetAccount(biller.PlayerData.job.name).."^7)") end
		elseif Config.Banking == "sgx-banking" then
			exports["sgx-banking"]:AddMoney(tostring(biller.PlayerData.job.name), data.amount - commission)
			if Config.Debug then print("^5Debug^7: ^3sgx-banking^7(^3Job^7): ^2Adding ^7$^6"..data.amount - takecomm.." ^2to account ^7'^6"..tostring(biller.PlayerData.job.name).."^7' ($^6"..exports["sgx-banking"]:GetAccount(biller.PlayerData.job.name).."^7)") end
		elseif Config.Banking == "fd" then
...
QBCore.Functions.CreateCallback('sgx-payments:MakePlayerList', function(source, cb)
	local onlineList = {}
	for _, v in pairs(QBCore.Functions.GetPlayers()) do
...
CreateThread(function()
	if Config.Usebzzz then
		...
		TriggerClientEvent("sgx-payments:client:Charge", source, {}, true)
		end)
	end
end)
