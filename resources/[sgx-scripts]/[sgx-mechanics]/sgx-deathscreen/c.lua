...
RegisterNetEvent('sgx-deathscreen:openUI', function(time)
    isDead = true
...
RegisterNetEvent('sgx-deathscreen:revive', function()
    isDead = false
...
RegisterNetEvent('sgx-deathscreen:die', function(time)
    isDead = true
    SendNUIMessage({action = "DSMenu", open = true, time = time})
    if Config.Blur then
        TriggerScreenblurFadeIn()
...
RegisterNetEvent('sgx-deathscreen:res', function()
    SendNUIMessage({action = "res"})
end)

RegisterNetEvent('sgx-deathscreen:updateRes', function(time)
    SendNUIMessage({action = "updateRes", time = time})
end)

RegisterCommand('test',function()
    TriggerEvent('sgx-deathscreen:updateRes', 0)
end)
