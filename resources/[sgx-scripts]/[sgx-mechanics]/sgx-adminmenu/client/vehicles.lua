...
-- Own Vehicle
RegisterNetEvent('sgx-adminmenu:client:Admincar', function(data)
    ...
    if sharedVehicles then
        TriggerServerEvent('sgx-adminmenu:server:SaveCar', props, sharedVehicles, hash, props.plate)
    else
...
-- Spawn Vehicle
RegisterNetEvent('sgx-adminmenu:client:SpawnVehicle', function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
...
-- Refuel Vehicle
RegisterNetEvent('sgx-adminmenu:client:RefuelVehicle', function(data)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
...
-- Change plate
RegisterNetEvent('sgx-adminmenu:client:ChangePlate', function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
    ...
    if cache.vehicle then
        local AlreadyPlate = lib.callback.await("sgx-adminmenu:callback:CheckAlreadyPlate", false, plate)
        ...
        TriggerServerEvent('sgx-adminmenu:server:ChangePlate', plate, currentPlate)
        Wait(100)
...
RegisterNetEvent('sgx-adminmenu:client:ToggleVehDevMenu', function(data)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
    if not cache.vehicle then return end
...
RegisterNetEvent('sgx-adminmenu:client:maxmodVehicle', function(data)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
...
RegisterNetEvent("sgx-adminmenu:client:SpawnPersonalVehicle", function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
    ...
    lib.callback('sgx-adminmenu:server:GetVehicleByPlate', false, function(vehModel)
        vehicle = vehModel
    end, plate)
...
-- Get Vehicle Data
lib.callback.register("sgx-adminmenu:client:getvehData", function(vehicle)
