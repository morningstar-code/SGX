...
local function preparePlayers()
    currentPlayers = {}
    Wait(100)
    currentPlayers = lib.callback.await('sgx-adminmenu:callback:GetPlayers')
end

-- Toggle Blips and Names events
RegisterNetEvent('sgx-adminmenu:client:toggleBlips', function(data)
...
RegisterNetEvent('sgx-adminmenu:client:toggleNames', function(data)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
...
-- Mute Player
RegisterNetEvent("sgx-adminmenu:client:MutePlayer", function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
...
-- Remove Stress
RegisterNetEvent('sgx-adminmenu:client:removeStress', function(data)
