...
-- Teleport to player
RegisterNetEvent('sgx-adminmenu:client:TeleportToPlayer', function(coords)
    lastCoords = GetEntityCoords(cache.ped)
    SetPedCoordsKeepVehicle(cache.ped, coords.x, coords.y, coords.z)
end)

-- Teleport to coords
RegisterNetEvent('sgx-adminmenu:client:TeleportToCoords', function(data, selectedData)
...
-- Teleport to Locaton
RegisterNetEvent('sgx-adminmenu:client:TeleportToLocation', function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
...
-- Teleport back
RegisterNetEvent('sgx-adminmenu:client:TeleportBack', function(data)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
