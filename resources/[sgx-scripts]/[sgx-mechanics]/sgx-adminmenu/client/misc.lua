...
local visible = true
RegisterNetEvent('sgx-adminmenu:client:ToggleInvisible', function(data)
...
-- God Mode
local godmode = false
RegisterNetEvent('sgx-adminmenu:client:ToggleGodmode', function(data)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
    godmode = not godmode
...
-- Cuff/Uncuff
RegisterNetEvent('sgx-adminmenu:client:ToggleCuffs', function(player)
...
RegisterNetEvent('sgx-adminmenu:client:setInfiniteAmmo', function(data)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
...
RegisterNetEvent('sgx-adminmenu:client:ToggleCoords', function(data)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
...
-- Set Ammo
RegisterNetEvent('sgx-adminmenu:client:SetAmmo', function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
...
RegisterNetEvent('sgx-adminmenu:client:ToggleDev', function(dataKey)
    local data = CheckDataFromKey(dataKey)
    if not data or not CheckPerms(data.perms) then return end

    ToggleDev = not ToggleDev

    TriggerEvent("qb-admin:client:ToggleDevmode")              -- toggle dev mode (sgx-hud/qb-hud)
    TriggerEvent('sgx-adminmenu:client:ToggleCoords', dataKey)  -- toggle Coords
    TriggerEvent('sgx-adminmenu:client:ToggleGodmode', dataKey) -- Godmode
...
-- Set Ped
RegisterNetEvent("sgx-adminmenu:client:setPed", function(pedModels)
