-- Ban Player
RegisterNetEvent('sgx-adminmenu:server:BanPlayer', function(data, selectedData)
...
-- Warn Player
RegisterNetEvent('sgx-adminmenu:server:WarnPlayer', function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(source, data.perms) then return end
...
RegisterNetEvent('sgx-adminmenu:server:KickPlayer', function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(source, data.perms) then return end
    local src = source
...
-- Revive Player
RegisterNetEvent('sgx-adminmenu:server:Revive', function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(source, data.perms) then return end
    local player = selectedData["Player"].value
...
-- Revive All
RegisterNetEvent('sgx-adminmenu:server:ReviveAll', function(data)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(source, data.perms) then return end
...
-- Revive Radius
RegisterNetEvent('sgx-adminmenu:server:ReviveRadius', function(data)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(source, data.perms) then return end
...
-- Set RoutingBucket
RegisterNetEvent('sgx-adminmenu:server:SetBucket', function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(source, data.perms) then return end
...
-- Get RoutingBucket
RegisterNetEvent('sgx-adminmenu:server:GetBucket', function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(source, data.perms) then return end
...
-- Give Money
RegisterNetEvent('sgx-adminmenu:server:GiveMoney', function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(source, data.perms) then return end
...
-- Give Money to all
RegisterNetEvent('sgx-adminmenu:server:GiveMoneyAll', function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(source, data.perms) then return end
...
-- Take Money
RegisterNetEvent('sgx-adminmenu:server:TakeMoney', function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(source, data.perms) then return end
...
RegisterNetEvent('sgx-adminmenu:server:ToggleBlackout', function(data)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(source, data.perms) then return end
...
-- Toggle Cuffs
RegisterNetEvent('sgx-adminmenu:server:CuffPlayer', function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(source, data.perms) then return end

    local target = selectedData["Player"].value

    TriggerClientEvent('sgx-adminmenu:client:ToggleCuffs', target)
    QBCore.Functions.Notify(source, locale("toggled_cuffs"), 'success')
end)

-- Give Clothing Menu
RegisterNetEvent('sgx-adminmenu:server:ClothingMenu', function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(source, data.perms) then return end
    ...
    if target == src then
        TriggerClientEvent("sgx-adminmenu:client:CloseUI", src)
    end
...
-- Set Ped
RegisterNetEvent("sgx-adminmenu:server:setPed", function(data, selectedData)
    local src = source
    local data = CheckDataFromKey(data)
    ...
    TriggerClientEvent("sgx-adminmenu:client:setPed", Player.PlayerData.source, ped)
end)
