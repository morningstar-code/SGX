...
lib.callback.register('sgx-adminmenu:callback:GetPlayers', function(source)
    return getPlayers()
end)

-- Set Job
RegisterNetEvent('sgx-adminmenu:server:SetJob', function(data, selectedData)
...
-- Set Gang
RegisterNetEvent('sgx-adminmenu:server:SetGang', function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(source, data.perms) then return end
    local src = source
...
-- Set Perms
RegisterNetEvent("sgx-adminmenu:server:SetPerms", function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(source, data.perms) then return end
    local src = source
...
-- Remove Stress
RegisterNetEvent("sgx-adminmenu:server:RemoveStress", function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(source, data.perms) then return end
    local src = source
    ...
    TriggerClientEvent('sgx-adminmenu:client:removeStress', targetId)
