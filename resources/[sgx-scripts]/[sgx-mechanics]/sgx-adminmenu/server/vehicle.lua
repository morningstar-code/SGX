-- Admin Car
RegisterNetEvent('sgx-adminmenu:server:SaveCar', function(mods, vehicle, _, plate)
    local src = source
...
-- Give Car
RegisterNetEvent("sgx-adminmenu:server:givecar", function(data, selectedData)
    local src = source
    ...
    local vehicleData = lib.callback.await("sgx-adminmenu:client:getvehData", src, vehmodel)
...
-- Give Car
RegisterNetEvent("sgx-adminmenu:server:SetVehicleState", function(data, selectedData)
    local src = source
...
-- Change Plate
RegisterNetEvent('sgx-adminmenu:server:ChangePlate', function(newPlate, currentPlate)
...
lib.callback.register('sgx-adminmenu:server:GetVehicleByPlate', function(source, plate)
...
RegisterNetEvent('sgx-adminmenu:server:FixVehFor', function(data, selectedData)
    local data = CheckDataFromKey(data)
