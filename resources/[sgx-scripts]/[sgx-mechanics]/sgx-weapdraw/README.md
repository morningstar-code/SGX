...
**Set aim animation:**
```lua
exports["sgx-weapdraw"]:setAimAnim("gang")
```

**Get aim animation**
```lua
exports["sgx-weapdraw"]:getAimAnim() -- default | gang | hillbilly
```
...
Open `sgx-weapdraw/data/aim.lua`, to create a command that changes aim animation you can do `command = "aim"` when player use /aim they can select their aim animation. If you want to disable command you can do `command = false`.
...
`sgx-weapdraw/data/holster.lua` is the config that changes the holster clothing model, you can set it to any clothing and any wepaon you'd like.
...
To configure the holster animations for different weapons you can do it in `sgx-weapdraw/data/animations.lua`
