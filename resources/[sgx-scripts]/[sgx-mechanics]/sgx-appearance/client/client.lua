...
local function LoadPlayerUniform()
    QBCore.Functions.TriggerCallback("sgx-appearance:server:getUniform", function(uniformData)
        if not uniformData then
            return
        end
        if Config.BossManagedOutfits then
            QBCore.Functions.TriggerCallback("sgx-appearance:server:getManagementOutfits", function(result)
                local uniform = nil
                for i = 1, #result, 1 do
                ...
                if not uniform then
                    TriggerServerEvent("sgx-appearance:server:syncUniform", nil) -- Uniform doesn't exist anymore
                    return
                end
    
                TriggerEvent("sgx-appearance:client:changeOutfit", uniform)
...
local function AddManagementMenuItems()
    local eventName = "sgx-appearance:client:OutfitManagementMenu"
    local menuItem = {
...
local function InitAppearance()
    ...
    QBCore.Functions.TriggerCallback('sgx-appearance:server:getAppearance', function(appearance)
...
RegisterNetEvent('qb-clothes:client:CreateFirstCharacter', function()
    QBCore.Functions.GetPlayerData(function(pd)
        ...
        exports[resourceName]:startPlayerCustomization(function(appearance)
            if (appearance) then
                TriggerServerEvent('sgx-appearance:server:saveAppearance', appearance)
                ResetRechargeMultipliers()
            end
        end, config)
    end)
...
function OpenShop(config, isPedMenu, shopType)
    QBCore.Functions.TriggerCallback("sgx-appearance:server:hasMoney", function(hasMoney, money)
        ...
        exports[resourceName]:startPlayerCustomization(function(appearance)
            if appearance then
                if not isPedMenu then
                    TriggerServerEvent("sgx-appearance:server:chargeCustomer", shopType)
                end
                TriggerServerEvent('sgx-appearance:server:saveAppearance', appearance)
            else
...
RegisterNetEvent('sgx-appearance:client:openClothingShop', OpenClothingShop)

RegisterNetEvent('sgx-appearance:client:saveOutfit', function()
    ...
    if keyboard ~= nil then
        Wait(500)
        QBCore.Functions.TriggerCallback("sgx-appearance:server:getOutfits", function(outfits)
            ...
            TriggerServerEvent('sgx-appearance:server:saveOutfit', keyboard.input, pedModel, pedComponents, pedProps)
        end)
    end
end)

RegisterNetEvent("sgx-appearance:client:OutfitManagementMenu", function(args)
    ...
    local menuItems = {
        ...
        },
        {
            ...
            params = {
                event = "sgx-appearance:client:ChangeManagementOutfitMenu",
                args = {
        ...
        },
        {
            ...
            params = {
                event = "sgx-appearance:client:SaveManagementOutfit",
                args = args.type
            }
        },
        {
            ...
            params = {
                event = "sgx-appearance:client:DeleteManagementOutfitMenu",
                args = {
                    backEvent = args.backEvent,
                    type = args.type,
                }
...
RegisterNetEvent("sgx-appearance:client:SaveManagementOutfit", function(mType)
    local playerPed = PlayerPedId()
    local outfitData = {
        Type = mType,
    ...
    TriggerServerEvent("sgx-appearance:server:saveManagementOutfit", outfitData)

end)

RegisterNetEvent("sgx-appearance:client:DeleteManagementOutfitMenu", function(args)
    QBCore.Functions.TriggerCallback('sgx-appearance:server:getManagementOutfits', function(result)
        local outfitMenu = {}
        for i = 1, #result, 1 do
            outfitMenu[#outfitMenu + 1] = {
                ...
                params = {
                    event = 'sgx-appearance:client:DeleteManagementOutfit',
                    args = result[i].id
                }
            }
        end
        outfitMenu[#outfitMenu + 1] = {
            ...
            icon = "fa-solid fa-angle-left",
            params = {
                event = "sgx-appearance:client:OutfitManagementMenu",
                args = {
                    backEvent = args.backEvent,
                    type = args.type,
                }
...
RegisterNetEvent("sgx-appearance:client:ChangeManagementOutfitMenu", function(args)
    QBCore.Functions.TriggerCallback('sgx-appearance:server:getManagementOutfits', function(result)
        local outfitMenu = {}
        for i = 1, #result, 1 do
            outfitMenu[#outfitMenu + 1] = {
                ...
                txt = result[i].model,
                params = {
                    event = 'sgx-appearance:client:changeOutfit',
                    args = {
                        type = args.type,
                        name = result[i].name,
                        model = result[i].model,
...
function OpenMenu(isPedMenu, backEvent, menuType, menuData)
    ...
    local outfitMenuItems = {{
        ...
        params = {
            event = "sgx-appearance:client:changeOutfitMenu",
            args = {
                isPedMenu = isPedMenu,
                backEvent = backEvent
            }
        ...
        params = {
            event = "sgx-appearance:client:saveOutfit"
        }
    }, {
        header = "Delete Outfit",
        txt = "Yeah... We didnt like that one either",
        params = {
            event = "sgx-appearance:client:deleteOutfitMenu",
            args = {
                isPedMenu = isPedMenu,
                backEvent = backEvent
            }
    ...
    if menuType == "default" then
        ...
        menuItems[#menuItems + 1] = {
            ...
            params = {
                event = "sgx-appearance:client:openClothingShop",
                args = isPedMenu
            }
        }
    ...
    elseif menuType == "job-outfit" then
        ...
        menuItems[#menuItems + 1] = {
            ...
            params = {
                event = "sgx-appearance:client:reloadSkin"
            }
        }
        menuItems[#menuItems + 1] = {
            header = "Work Clothes",
            ...
            params = {
                event = "sgx-appearance:client:openJobOutfitsListMenu",
                args = {
                    backEvent = backEvent,
                    menuData = menuData
                }
...
RegisterNetEvent("sgx-appearance:client:openJobOutfitsListMenu", function(data)
    local menu = {{
        header = '< Go Back',
        params = {
...
RegisterNetEvent("sgx-appearance:client:openClothingShopMenu", function(isPedMenu)
    if type(isPedMenu) == "table" then
        isPedMenu = false
    end
    OpenMenu(isPedMenu, "sgx-appearance:client:openClothingShopMenu", "default")
end)

RegisterNetEvent("sgx-appearance:client:OpenBarberShop", function()
    OpenBarberShop()
end)

RegisterNetEvent("sgx-appearance:client:OpenTattooShop", function()
    OpenTattooShop()
end)

RegisterNetEvent("sgx-appearance:client:OpenSurgeonShop", function()
    OpenSurgeonShop()
end)

RegisterNetEvent("sgx-appearance:client:changeOutfitMenu", function(data)
    QBCore.Functions.TriggerCallback('sgx-appearance:server:getOutfits', function(result)
        local outfitMenu = {{
            header = '< Go Back',
            params = {
                event = data.backEvent,
...
RegisterNetEvent("sgx-appearance:client:changeOutfit", function(data)
    local playerPed = PlayerPedId()
    local pedModel = exports[resourceName]:getPedModel(playerPed)
    local failed = false
...
RegisterNetEvent("sgx-appearance:client:deleteOutfitMenu", function(data)
    QBCore.Functions.TriggerCallback('sgx-appearance:server:getOutfits', function(result)
        local outfitMenu = {{
            header = '< Go Back',
            params = {
        ...
        for i = 1, #result, 1 do
            outfitMenu[#outfitMenu + 1] = {
                ...
                params = {
                    event = 'sgx-appearance:client:deleteOutfit',
                    args = result[i].id
                }
            }
        end
...
RegisterNetEvent("sgx-appearance:client:DeleteManagementOutfit", function(id)
    TriggerServerEvent("sgx-appearance:server:deleteManagementOutfit", id)
    QBCore.Functions.Notify('Outfit Deleted', 'error')
end)

RegisterNetEvent('sgx-appearance:client:deleteOutfit', function(id)
    TriggerServerEvent('sgx-appearance:server:deleteOutfit', id)
    QBCore.Functions.Notify('Outfit Deleted', 'error')
end)

RegisterNetEvent('sgx-appearance:client:openJobOutfitsMenu', function(outfitsToShow)
    OpenMenu(nil, "sgx-appearance:client:openJobOutfitsMenu", "job-outfit", outfitsToShow)
end)
...
RegisterNetEvent('sgx-appearance:client:reloadSkin', function()
    ...
    QBCore.Functions.TriggerCallback('sgx-appearance:server:getAppearance', function(appearance)
        ...
        exports[resourceName]:setPlayerAppearance(appearance)
        if Config.PersistUniforms then
            TriggerServerEvent("sgx-appearance:server:syncUniform", nil)
        end
        playerPed = PlayerPedId()
...
RegisterNetEvent("qb-radialmenu:client:onRadialmenuOpen", function()
    ...
    if string.find(zoneName, "ClothingRooms_") then
        event = "sgx-appearance:client:OpenClothingRoom"
        title = "Clothing Room"
    elseif string.find(zoneName, "PlayerOutfitRooms_") then
        event = "sgx-appearance:client:OpenPlayerOutfitRoom"
        title = "Player Outfits"
    elseif zoneName == "clothing" then
        event = "sgx-appearance:client:openClothingShopMenu"
        title = "Clothing Shop"
    elseif zoneName == "barber" then
        event = "sgx-appearance:client:OpenBarberShop"
        title = "Barber Shop"
    elseif zoneName == "tattoo" then
        event = "sgx-appearance:client:OpenTattooShop"
        title = "Tattoo Shop"
    elseif zoneName == "surgeon" then
        event = "sgx-appearance:client:OpenSurgeonShop"
        title = "Surgeon Shop"
    end
...
RegisterNetEvent("sgx-appearance:client:OpenClothingRoom", function()
    local clothingRoom = Config.ClothingRooms[tonumber(string.sub(zoneName, 15))]
    local outfits = getPlayerJobOutfits(clothingRoom)
    TriggerEvent('sgx-appearance:client:openJobOutfitsMenu', outfits)
end)

RegisterNetEvent("sgx-appearance:client:OpenPlayerOutfitRoom", function()
    ...
    storeCombo:onPlayerInOut(function(isPointInside, _, zone)
        if isPointInside then
            ...
            if jobName == (currentStore.job or currentStore.gang) then
                inZone = true
                local prefix = Config.UseRadialMenu and '' or ''
...
local function SetupStoreTargets()
    for k, v in pairs(Config.Stores) do
        ...
        elseif v.shopType == 'clothing' then
            action = function()
                TriggerEvent("sgx-appearance:client:openClothingShopMenu")
            end
