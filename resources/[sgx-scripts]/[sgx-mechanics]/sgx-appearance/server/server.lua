...
QBCore.Functions.CreateCallback('sgx-appearance:server:getAppearance', function(source, cb, model)
...
QBCore.Functions.CreateCallback('sgx-appearance:server:hasMoney', function(source, cb, shopType)
    local Player = QBCore.Functions.GetPlayer(source)
...
QBCore.Functions.CreateCallback('sgx-appearance:server:getOutfits', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
...
QBCore.Functions.CreateCallback("sgx-appearance:server:getManagementOutfits", function(source, cb, mType, gender)
    local Player = QBCore.Functions.GetPlayer(source)
...
QBCore.Functions.CreateCallback("sgx-appearance:server:getUniform", function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
    cb(uniformCache[Player.PlayerData.citizenid])
end)

RegisterServerEvent("sgx-appearance:server:saveAppearance", function(appearance)
    local src = source
...
RegisterServerEvent("sgx-appearance:server:chargeCustomer", function(shopType)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local money = getMoneyForShop(shopType)
...
RegisterNetEvent('sgx-appearance:server:saveOutfit', function(name, model, components, props)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if outfitCache[Player.PlayerData.citizenid] == nil then
        getOutfitsForPlayer(Player.PlayerData.citizenid)
...
RegisterNetEvent("sgx-appearance:server:saveManagementOutfit", function(outfitData)
    local src = source
...
RegisterNetEvent("sgx-appearance:server:deleteManagementOutfit", function(id)
    MySQL.query("DELETE FROM management_outfits WHERE id = ?", {id})
end)

RegisterNetEvent("sgx-appearance:server:syncUniform", function(uniform)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    uniformCache[Player.PlayerData.citizenid] = uniform
end)

RegisterNetEvent('sgx-appearance:server:deleteOutfit', function(id)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent("sgx-appearance:server:resetOutfitCache", function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player then
...
if Config.EnablePedMenu then
    QBCore.Commands.Add('pedmenu', 'Open Ped Menu', {{
        ...
        TriggerClientEvent("sgx-appearance:client:openClothingShopMenu", src, true)
    end, Config.PedMenuGroup)
end

QBCore.Commands.Add('reloadskin', 'Reloads your character', {}, false, function(source, _)
    TriggerClientEvent("sgx-appearance:client:reloadSkin", source)
end)
