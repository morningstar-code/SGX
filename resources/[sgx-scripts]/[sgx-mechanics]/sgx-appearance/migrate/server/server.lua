...
local function MigrateQBClothing(source)
    ...
    for i=1, #allPlayerSkins, 1 do
        if not tonumber(allPlayerSkins[i].model) then
            ...
            TriggerClientEvent("sgx-appearance:client:migration:load-qb-clothing-skin", source, allPlayerSkins[i])
...
RegisterNetEvent("sgx-appearance:server:migrate-qb-clothing-skin", function(citizenid, appearance)
    local src = source
...
QBCore.Commands.Add('migrateskins', 'Migrate skins to sgx-appearance', {{name='type', help='sgx-appearance / qb-clothing'}}, false, function(source, args)
    local type = tostring(args[1])
    if type == "sgx-appearance" then
