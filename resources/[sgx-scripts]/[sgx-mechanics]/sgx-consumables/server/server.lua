...
for k, v in pairs(Config.Consumables) do
	QBCore.Functions.CreateUseableItem(k, function(source, item) TriggerClientEvent('sgx-consumables:Consume', source, item.name) end)
...
RegisterNetEvent("sgx-consumables:server:DupeWarn", function(item, newsrc)
...
RegisterNetEvent('sgx-consumables:server:toggleItem', function(give, item, amount, newsrc)
	local src = newsrc or source
	...
	if not give then
		...
		else TriggerEvent("sgx-consumables:server:DupeWarn", item, src) end -- if not boot the player
	elseif give then
...
RegisterNetEvent('sgx-consumables:server:addNeed', function(amount, type)
...
QBCore.Functions.CreateCallback('sgx-consumables:server:syncConsumables', function(source, cb) cb(Consumables) end)
QBCore.Functions.CreateCallback('sgx-consumables:server:syncEmotes', function(source, cb) cb(Emotes) end)

RegisterNetEvent('sgx-consumables:server:syncAddItem', function(itemName, data)
	if not Consumables[itemName] then
		QBCore.Functions.CreateUseableItem(itemName, function(source, item) TriggerClientEvent('sgx-consumables:Consume', source, itemName) end)
		if Config.Debug then print("^5Debug^7: ^2Adding new ^3Item^7: '^6"..itemName.."^7'") end
		Consumables[itemName] = data
		TriggerClientEvent("sgx-consumables:client:syncConsumables", -1, Consumables)
	end
end)

RegisterNetEvent('sgx-consumables:server:syncAddEmote', function(emoteName, data)
	if not Emotes[emoteName] then
		...
		TriggerClientEvent("sgx-consumables:client:syncEmotes", -1, Emotes)
	end
end)

RegisterNetEvent("sgx-consumables:server:syncConsumables", function() TriggerClientEvent('sgx-consumables:client:syncConsumables', -1, Consumables) end)
RegisterNetEvent("sgx-consumables:server:syncEmotes", function() TriggerClientEvent('sgx-consumables:client:syncEmotes', -1, Emotes) end)

local function CheckVersion()
	PerformHttpRequest('https://raw.githubusercontent.com/jimathy/sgx-consumables/master/version.txt', function(err, newestVersion, headers)
