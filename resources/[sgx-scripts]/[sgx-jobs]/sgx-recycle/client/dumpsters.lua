if Config.DumpsterDiving.Enable then if Config.Debug then print("^5Debug^7: ^2Loading^7: '^6Dumpster Diving^7'") end
    ...
    exports['qb-target']:AddTargetModel(Config.DumpsterDiving.models,
        { options = {
            {
                action = function(entity) TriggerEvent("sgx-recycle:Dumpsters:Search", { entity = entity }) end,
    ...
    RegisterNetEvent('sgx-recycle:Dumpsters:Search', function(data) local Ped = PlayerPedId()
        ...
        if not searched then -- If hasn't been searched yet
            ...
            elseif Config.DumpsterDiving.skillcheck == "sgx-repairkit" then
                local Skillbar = exports["sgx-repairkit"]:Minigame()
                if Skillbar then searchSuccess = true else searchSuccess = false end
            else
                searchSuccess = true
            end
