...
Core.Functions.CreateCallback('sgx-recycle:GetCash', function(source, cb) cb(Core.Functions.GetPlayer(source).Functions.GetMoney("cash")) end)

RegisterServerEvent("sgx-recycle:DoorCharge", function()
	if Config.Inv == "ox" then TriggerEvent("sgx-recycle:server:toggleItem", false, "money", Config.PayAtDoor, src)
	else Core.Functions.GetPlayer(source).Functions.RemoveMoney("cash", Config.PayAtDoor) end
end)

RegisterServerEvent("sgx-recycle:TradeItems", function(data)
    local src = source
	local table = {}
	...
	TriggerEvent("sgx-recycle:server:toggleItem", false, "recyclablematerial", data.amount, src)
	Wait(1000)
	for i = 1, table.itemGive do
		TriggerEvent("sgx-recycle:server:toggleItem", true, Config.TradeTable[math.random(1, #Config.TradeTable)], math.random(table.Min, table.Max), src)
		Wait(100)
	end
end)

RegisterNetEvent("sgx-recycle:Selling:Mat", function(data)
    local src = source
	...
	if HasItem(src, data.item, amount) then
		TriggerEvent("sgx-recycle:server:toggleItem", false, data.item, amount, src)
	...
	TriggerClientEvent(src, "sgx-recycle:Selling:Menu", data)
end)
...
RegisterNetEvent('sgx-recycle:server:toggleItem', function(give, item, amount, newsrc)
