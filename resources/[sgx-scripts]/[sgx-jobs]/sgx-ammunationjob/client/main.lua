...
RegisterNetEvent('sgx-ammunation:Client:Invoicing', function()
    print("Invoicing event triggered") 
    if Config.SGXPayments then
        print("Triggering SGXPayments") 
        TriggerEvent("sgx-payments:client:Charge", Config.Job)
    else
        ...
        if dialog then
            ...
            TriggerServerEvent("sgx-ammunation:Server:Billing", dialog.id, dialog.amount)
        else
    ...
    local menuItems = {
        {header = Language.Menu.CraftingCatalog, isMenuHeader = true},
        {
            ...
            params = {event = "sgx-ammunation:client:OpenWeaponsCraftMenu"}
        },
        {
            header = Language.Menu.Ammo,
            txt = Language.Menu.AmmoDescription,
            params = {event = "sgx-ammunation:client:OpenAmmoCraftMenu"}
        },
        {
    ...
    RegisterNetEvent('sgx-ammunation:client:OpenWeaponsCraftMenu', function()
        CraftWeaponsMenu()
    end)
    
    RegisterNetEvent('sgx-ammunation:client:OpenAmmoCraftMenu', function()
        CraftAmmoMenu()
    end)
    ...
    function CraftWeaponsMenu()
        ...
        for k, v in pairs(Config.Weapons) do
            ...
            item.text = text
            item.params = {
                event = 'sgx-ammunation:client:CraftWeapons',
                args = { type = k }
            }
        ...
        table.insert(columns, {
            ...
            txt = "",
            params = {
                event = "sgx-ammunation:client:OpenCatalogMenu" 
            }
        })
...
RegisterNetEvent('sgx-ammunation:client:OpenCatalogMenu', function()
    CraftCatalogMenu()
end)
...
function CraftWeapons(weapon)
    if lib.progressCircle({
        ...
        if lib.progressCircle({
            ...
            TriggerServerEvent('sgx-ammunation:server:CraftWeapons', Config.Weapons[weapon].hash)
            for k, v in pairs(Config.Weapons[weapon].materials) do
                TriggerServerEvent('sgx-ammunation:Server:RemoveItem', v.item, v.amount)
...
RegisterNetEvent('sgx-ammunation:client:CraftWeapons', function(data)
    QBCore.Functions.TriggerCallback("sgx-ammunation:server:Materials", function(hasMaterials, missingMaterials)
...
function CraftAmmoMenu()
    ...
    for k, v in pairs(Config.Ammo) do
        ...
        item.text = text
        item.params = { event = 'sgx-ammunation:client:CraftAmmo', args = { type = k } }
        table.insert(columns, item)
    end
...
function CraftAmmo(weapon)
    QBCore.Functions.Progressbar("combine_parts", "Combining The Parts...", 6000, true, true, {
        ...
        anim = "base",
        flags = 49, -- Adjust this flag if needed to fit the animation requirements
    }, {
        model = "prop_tool_hammer",
        bone = 57005,
        coords = { x = 0.12, y = 0.0, z = -0.02 },
        rotation = { x = 90.0, y = 0.0, z = 0.0 },
    }, {}, function() -- On success
        QBCore.Functions.Progressbar("make_ammo", Language.Progressbars.Make..Config.Ammo[weapon].label, 9500, true, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
            ...
            TriggerServerEvent('sgx-ammunation:server:CraftAmmo', Config.Ammo[weapon].hash)
...
RegisterNetEvent('sgx-ammunation:client:CraftAmmo', function(data)
    QBCore.Functions.TriggerCallback("sgx-ammunation:server:Materials", function(hasMaterials, missingMaterials)
        if hasMaterials then
            CraftAmmo(data.type)
        else
